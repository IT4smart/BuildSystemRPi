...
# Introdruce settings
set -e
echo -n -e "\n#\n# Management-Agent Bootstrap Settings\n#\n"
set -x
...
function prepare_build_env() {  
    ...
    APT_INCLUDES=apt-transport-https,apt-utils,ca-certificates,dialog,sudo,git,build-essential,bc,dh-systemd,python-virtualenv,python2.7,python2.7-dev,dh-virtualenv
...
# get git repository Verwaltungskonsole-Agent
git clone http://build.service:123456@devbase.it4s.eu:3000/IT4S/Verwaltungskonsole-Agent.git "${R}${SRC_DIR}/Verwaltungskonsole-Agent"

# Get latest tag name
#latestTag=$(git describe --tags `git rev-list --tags --max-count=1`)

# create virtual environment
chroot_exec virtualenv "${SRC_DIR}/virt-example" --no-site-packages

# build 
echo -e "Building IT4S - Management-Agent for $1"
#chroot_exec export DH_VIRTUALENV_INSTALL_ROOT=/opt/IT4S
# We build the package by hand
chroot_exec 
